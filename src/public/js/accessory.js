...
sortAsc.onclick = () => {
    if (
        ...
        window.history.pushState({}, "", "http://localhost:3000/accessory?sort=asc");
        window.location.reload();
    }
};
sortDesc.onclick = () => {
    if (
        ...
        window.history.pushState({}, "", "http://localhost:3000/accessory?sort=desc");
        window.location.reload();
    }
};
...
buyBtn.forEach((buyBtnitem) => {
    buyBtnitem.onsubmit = function (e) {
        ...
        fetch("http://localhost:3000/account/login/user", {
            ...
            .then((res) => {
                if (res.status == "true") {
                    fetch("http://localhost:3000/accessory/checkout?itemID=" + buyBtnitem.getAttribute("data"), {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json",
                        },
...
addBtn.forEach((addBtnitem) => {
    addBtnitem.onsubmit = function (e) {
        ...
        fetch("http://localhost:3000/account/login/user", {
            ...
            .then((res) => {
                if (res.status == "true") {
                    fetch("http://localhost:3000/accessory/checkout?itemID=" + addBtnitem.getAttribute("data"), {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json",
                        },
