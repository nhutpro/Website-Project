...
if (document.referrer == "/accessory") {
  checkBoxAll.checked = true;
  for (var checkbox of brandidCheckboxes) {
    checkbox.checked = true;
  }
...
sortAsc.onclick = () => {
  if (
    ...
    window.history.pushState({}, "", "/accessory?sort=asc");
    window.location.reload();
  }
};
sortDesc.onclick = () => {
  if (
    ...
    window.history.pushState({}, "", "/accessory?sort=desc");
    window.location.reload();
  }
};
...
// var api = "/accessory/totalproduct";
// var total;
// fetch(api, {
//     headers: {
  ...
  buyBtnitem.onsubmit = function (e) {
    ...
    fetch("/account/login/user", {
      ...
      .then((res) => {
        if (res.status == "true") {
          fetch(
            "/accessory/checkout?itemID=" + buyBtnitem.getAttribute("data"),
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
  ...
  addBtnitem.onsubmit = function (e) {
    ...
    fetch("/account/login/user", {
      ...
      .then((res) => {
        if (res.status == "true") {
          fetch(
            "/accessory/checkout?itemID=" + addBtnitem.getAttribute("data"),
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
