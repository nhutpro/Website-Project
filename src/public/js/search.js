		...
		buyBtn.forEach((buyBtnitem) => {
			buyBtnitem.onsubmit = function (e) {
				...
				fetch("http://localhost:3000/account/login/user", {
					...
					.then((res) => {
						if (res.status == "true") {
							fetch("http://localhost:3000/" + buyBtnitem.id + "/checkout?itemID=" + buyBtnitem.getAttribute("data"), {
								method: "POST",
								headers: {
									"Content-Type": "application/json",
								},
		...
		addBtn.forEach((addBtnitem) => {
			addBtnitem.onsubmit = function (e) {
				...
				fetch("http://localhost:3000/account/login/user", {
					...
					.then((res) => {
						if (res.status == "true") {
							fetch("http://localhost:3000/" + addBtnitem.id + "/checkout?itemID=" + addBtnitem.getAttribute("data"), {
								method: "POST",
								headers: {
									"Content-Type": "application/json",
								},
