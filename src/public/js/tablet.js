...
checkBoxAll.onclick = function () {
  ...
  //window.history.pushState({}, "", "http://team-13.herokuapp.com/phone");
...
if (document.referrer == "http://team-13.herokuapp.com/tablet") {
  checkBoxAll.checked = true;
  for (var checkbox of brandidCheckboxes) {
    checkbox.checked = true;
  }
...
sortAsc.onclick = () => {
  if (
    ...
    window.history.pushState(
      {},
      "",
      "http://team-13.herokuapp.com/tablet?sort=asc"
    );
    window.location.reload();
  }
};
sortDesc.onclick = () => {
  if (
    ...
    window.history.pushState(
      {},
      "",
      "http://team-13.herokuapp.com/tablet?sort=desc"
    );
    window.location.reload();
  }
};
...
page1.onclick = () => {
  window.history.pushState({}, "", "/tablet?page=1");
  window.location.reload();
...
page2.onclick = () => {
  window.history.pushState({}, "", "/tablet?page=2");
  window.location.reload();
...
page3.onclick = () => {
  window.history.pushState({}, "", "/tablet?page=3");
  window.location.reload();
  localStorage.setItem("page1", false);
  localStorage.setItem("page2", false);
...
page4.onclick = () => {
  window.history.pushState({}, "", "/tablet?page=4");
  window.location.reload();
  localStorage.setItem("page1", false);
  localStorage.setItem("page2", false);
  localStorage.setItem("page3", false);
...
var api = "http://team-13.herokuapp.com/tablet/totalproduct";
var total;
fetch(api, {
  headers: {
...
pageLast.onclick = () => {
  ...
  window.history.pushState({}, "", "/tablet?page=" + total);
  window.location.reload();
  localStorage.setItem("page1", false);
  localStorage.setItem("page2", false);
  localStorage.setItem("page3", false);
  ...
  buyBtnitem.onsubmit = function (e) {
    ...
    fetch("http://team-13.herokuapp.com/account/login/user", {
      ...
      .then((res) => {
        if (res.status == "true") {
          fetch(
            "http://team-13.herokuapp.com/tablet/checkout?itemID=" +
